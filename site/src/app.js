...
async function renderDashboard(app) {
  ...
  async function loadDashboardActivity(force = false) {
    ...
    activityInFlight = (async () => {
      ...
      const txRows = (data?.transactions?.nodes || []).reverse();
      const cpRows = (data?.checkpoints?.nodes || []).reverse();
      applyDashboardActivity(txRows, cpRows);
  ...
  app.innerHTML = `
    ...
    <div class="dash-grid-6">
      ...
      <div class="stat-box">
        ...
        <div class="sparkline-wrap" id="dash-tx-delta-spark"><span class="u-fs11-dim">loading...</span></div>
      </div>
      <div class="stat-box">
      ...
      <div class="card">
        ...
        <div class="card-body">
          <table>
            ...
            <tbody id="dash-cp-tbody">${renderCpRows([])}</tbody>
          </table>
      ...
      <div class="card">
        ...
        <div class="card-body">
          <table>
            ...
            <tbody id="dash-tx-tbody">${renderTxRows([])}</tbody>
          </table>
  ...
  loadDashboardActivity().catch(() => null);
  ...
  dashboardTimer = setInterval(async () => {
    ...
    try {
      ...
      if (cpTimeEl) {
        cpTimeEl.textContent = timeAgo(fcp.timestamp);
        cpTimeEl.title = fmtTime(fcp.timestamp);
      }
      const seq = Number(fcp?.sequenceNumber || 0);
      if (Number.isFinite(seq) && seq > 0 && seq !== lastHeadSeq) {
        lastHeadSeq = seq;
        loadDashboardActivity().catch(() => null);
...
async function renderCoin(app, routeCoinType = "") {
  ...
  const shouldScanActivity = routeParams.get("scan") === "1";
  ...
  try {
    ...
    if (coinMeta?.supply != null) {
      ...
      supplySource = "pending";
      supplyUnavailableReason = "Deferred on-chain lookup.";
    }
    ...
    function readSupplySnapshot() {
      const exact = hasSupply ? scaledBigIntToText(supplyRaw, decimals, 8) : "Unavailable";
      const approx = hasSupply ? scaledBigIntAbsToApprox(supplyRaw, decimals, 8) : NaN;
      const display = hasSupply
        ? (Number.isFinite(approx) ? fmtCompact(approx) : exact)
        : (supplyLookupPending ? "Loading..." : "—");
      const statSub = hasSupply
        ? `${symbol} units (${supplySource || "metadata"}${supplyIsEstimated ? ", derived" : ""})`
        : (supplyLookupPending ? "Resolving supply from on-chain sources..." : (supplyUnavailableReason || "coinMetadata.supply unavailable"));
      const source = hasSupply
        ? `${supplySource || "coinMetadata.supply"}${supplySourceNote ? ` (${supplySourceNote})` : ""}`
        : (supplyLookupPending ? "Pending on-chain lookup..." : (supplyUnavailableReason || "Unavailable"));
      return { exact, display, statSub, source };
    }

    function applySupplySnapshotToDom() {
      const snap = readSupplySnapshot();
      const statVal = document.getElementById("coin-supply-stat-value");
      const statSub = document.getElementById("coin-supply-stat-sub");
      const exactEl = document.getElementById("coin-supply-exact");
      const sourceEl = document.getElementById("coin-supply-source");
      if (statVal) statVal.textContent = snap.display;
      if (statSub) statSub.textContent = snap.statSub;
      if (exactEl) exactEl.textContent = hasSupply ? `${snap.exact} ${symbol}` : "Unavailable";
      if (sourceEl) sourceEl.textContent = snap.source;
    }
    ...
    if (!shouldScanActivity) {
          ...
          <div class="card-body">
            <div class="stats-grid u-mb16">
              ...
              <div class="stat-box">
                ...
                <div class="stat-value" id="coin-supply-stat-value">${escapeHtml(snap.display)}</div>
                <div class="stat-sub" id="coin-supply-stat-sub">${escapeHtml(snap.statSub)}</div>
              </div>
              <div class="stat-box">
            ...
            <div class="detail-grid">
              ...
              <div class="detail-row">
                ...
                <div class="detail-val" id="coin-supply-exact">${hasSupply ? `${escapeHtml(snap.exact)} ${escapeHtml(symbol)}` : '<span class="u-c-dim">Unavailable</span>'}</div>
              </div>
              <div class="detail-row">
                <div class="detail-key">Supply Source</div>
                <div class="detail-val" id="coin-supply-source">${escapeHtml(snap.source)}</div>
              </div>
              <div class="detail-row">
      ...
      if (supplyLookupPending) {
        fetchCoinTotalSupplyRpc(coinType, shortCoinType).then((rpcSupply) => {
          ...
          supplyUnavailableReason = "coinMetadata.supply unavailable";
          applySupplySnapshotToDom();
        });
      }
      return;
    ...
    const scanCacheKey = `${targetKey}|mode:${scanMode}|v1`;
