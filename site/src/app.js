  ...
  app.innerHTML = `
    ...
    <div class="dash-grid-6">
      ...
      <div class="stat-box">
        ...
        <div class="stat-value" data-stat="total-txns">${fmtCompact(cp.networkTotalTransactions)}</div>
        <div class="stat-sub">Epoch: ${fmtCompact(ep.totalTransactions)}</div>
...
async function fetchSuilendPositions(addr) {
  ...
  for (let i = 0; i < obligationIds.length; i++) {
    ...
    const borrows = (ob.borrows || []).map(b => {
      ...
      const dec = getDecimals(sym);
      const raw = Number(b.borrowed_amount?.value || 0);
      const amount = raw > 0 ? raw / Math.pow(10, dec) : (usd > 0 && defiPrices[sym] > 0 ? usd / defiPrices[sym] : 0);
      return { symbol: sym, amount, amountUsd: usd };
    });
...
async function fetchAlphaPositions(addr) {
  ...
  for (let i = 0; i < posIds.length; i++) {
    ...
    const deps = collContents.map(c => { const m = Number(c.key || 0); const sym = ALPHA_MARKETS[m] || `Market${m}`; const usdPer = collContents.length > 0 ? depositedUsd / collContents.length : 0; const amt = usdPer > 0 && defiPrices[sym] > 0 ? usdPer / defiPrices[sym] : 0; return { symbol: sym, amount: amt, amountUsd: usdPer }; });
    const loans = (data.loans || []).map(l => { const sym = String(l.coin_type?.name || "").split("::").pop() || "?"; const dec = getDecimals(sym); const human = Number(l.amount || 0) / Math.pow(10, dec); const usd = human * (defiPrices[sym] || 0); return { symbol: sym, amount: human, amountUsd: usd || (borrowedUsd / Math.max((data.loans || []).length, 1)) }; });
    ...
    if (lstBals.length) {
      ...
      for (const b of lstBals) {
        ...
        html += `<tr>
          ...
          <td style="font-family:var(--mono);font-size:12px;color:var(--text-dim)">${b.suiEquiv > 0 ? (b.suiEquiv >= 1000 ? fmtCompact(b.suiEquiv) : b.suiEquiv >= 1 ? b.suiEquiv.toFixed(1) : b.suiEquiv.toLocaleString(undefined, {maximumFractionDigits:4})) + " SUI" : "—"}<span class="u-fs10-dim">${rateLabel}</span></td>
      ...
      for (const pos of allLending) {
        ...
        if (pos.deposits?.length) {
          ...
          for (const d of pos.deposits) {
            ...
            html += `<div style="display:grid;grid-template-columns:72px 1fr auto;gap:4px;font-size:13px;padding:2px 0;align-items:baseline"><span class="u-fw-600">${d.symbol}</span><span style="font-family:var(--mono);font-size:12px;color:var(--text-dim)">${fmtAmt ? fmtAmt + " " + d.symbol : ""}</span><span class="u-c-green" style="font-family:var(--mono);font-size:12px;text-align:right">${d.amountUsd > 0 ? fmtUsdFromFloat(d.amountUsd) : "—"}</span></div>`;
          }
        }
        if (pos.borrows?.length) {
          ...
          for (const b of pos.borrows) {
            ...
            html += `<div style="display:grid;grid-template-columns:72px 1fr auto;gap:4px;font-size:13px;padding:2px 0;align-items:baseline"><span class="u-fw-600">${b.symbol}</span><span style="font-family:var(--mono);font-size:12px;color:var(--text-dim)">${fmtAmt ? fmtAmt + " " + b.symbol : ""}</span><span class="u-c-red" style="font-family:var(--mono);font-size:12px;text-align:right">${b.amountUsd > 0 ? fmtUsdFromFloat(b.amountUsd) : "—"}</span></div>`;
          }
        }
...
async function renderEvents(app) {
  ...
  async function loadEvents() {
    ...
    const data = await gql(`{ events(last: 50 ${filterArg}) { nodes { contents { type { repr } json } sender { address } timestamp transaction { digest } transactionModule { name package { address } } } } }`);
    ...
    if (!eventsData.length) {
      ...
      for (const group of txGroups) {
        ...
        for (const ev of group.events) {
          ...
          if (hasJson) {
            html += `<tr id="${rowId}" style="display:none"><td colspan="3" style="padding:0 8px 8px">${jsonTreeBlock(ev.contents.json, 280)}</td></tr>`;
          }
        }
