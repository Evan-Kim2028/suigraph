...
const PAGE_PERF_BUDGETS = Object.freeze({
  ...
  "defi-risk": { gqlCalls: 20, renderMs: 2500 },
...
function notePerfGql(elapsedMs, reqBytes, resBytes) {
  if (!pagePerf) return;
  pagePerf.gqlCalls++;
...
const DEFI_RISK_TTL_MS = 45 * 1000;
...
async function gql(query, variables = {}) {
  ...
  notePerfGql(elapsed, reqBytes, raw.length);
  let json = {};
  try {
...
function parseSortableCellValue(text) {
  ...
  if (!lower.startsWith("0x")) {
    ...
    const compactCandidate = lower
      .replace(/[$,%]/g, "")
      .replace(/,/g, "")
      .match(/[-+]?\d+(?:\.\d+)?[kmbt]\b/i)?.[0];
...
async function prefetchCoinMeta(coinTypes) {
  ...
  for (const chunk of chunkArray(toFetch, 20)) {
...
function parseRoute(route) {
  ...
  if (parts[0] === "defi-risk") return { page: "defi-risk" };
...
async function routeTo(route) {
  ...
  try {
    switch (r.page) {
      ...
      case "defi-risk": await renderDefiRisk(app); break;
...
async function renderDashboard(app) {
  ...
  const cp = dashData.checkpoint;
  const ep = dashData.epoch;
  const txs = dashData.transactions.nodes.reverse();
  const cpList = dashData.checkpoints.nodes.reverse();
  const checkpointTxDeltas = [];
  for (let i = 1; i < cpList.length; i++) {
    const prev = Number(cpList[i - 1]?.networkTotalTransactions || 0);
    const cur = Number(cpList[i]?.networkTotalTransactions || 0);
    if (Number.isFinite(prev) && Number.isFinite(cur) && cur >= prev) checkpointTxDeltas.push(cur - prev);
  }
  ...
  // Checkpoint row: compute tx count from networkTotalTransactions delta
  function renderCpRows(cps) {
    return cps.map((c, i) => {
      const prev = cps[i + 1]; // older checkpoint
    ...
    }).slice(0, -1).join(""); // drop the last one (only used for delta)
  }
  ...
  app.innerHTML = `
    ...
    <div class="dash-grid-6">
      <div class="stat-box">
        ...
        <div class="stat-value" data-stat="checkpoint">${fmtNumber(cp.sequenceNumber)}</div>
        <div class="stat-sub" data-stat-sub="cp-time" title="${fmtTime(cp.timestamp)}" style="cursor:help">${timeAgo(cp.timestamp)}</div>
      </div>
      <a class="stat-box" href="#/epoch/${ep.epochId}" style="display:block;cursor:pointer;text-decoration:none;color:inherit">
        <div class="stat-label">Current Epoch</div>
        <div class="stat-value u-c-accent" data-stat="epoch">${fmtNumber(ep.epochId)}</div>
        <div class="stat-sub">Started ${fmtTime(ep.startTimestamp)}</div>
      </a>
      <div class="stat-box">
        <div class="stat-label">Total Transactions</div>
        <div class="stat-value" data-stat="total-txns">${fmtNumber(cp.networkTotalTransactions)}</div>
        <div class="stat-sub">Epoch: ${fmtNumber(ep.totalTransactions)}</div>
        <div class="sparkline-wrap">${renderSparkline(checkpointTxDeltas, { prefix: "Δ ", suffix: " tx/checkpoint", color: "var(--blue)" })}</div>
      </div>
      <div class="stat-box">
        <div class="stat-label">Gas Price</div>
        <div class="stat-value">${fmtNumber(ep.referenceGasPrice)} <span class="u-fs12-dim">MIST</span></div>
    ...
    <!-- Section 3: Stablecoin Supply + TVL Breakdown -->
      ...
      <div class="card">
        ...
        <div class="card-body">
          <table>
            ...
            <tbody id="dash-cp-tbody">${renderCpRows(cpList)}</tbody>
          </table>
      ...
      <div class="card">
        ...
        <div class="card-body">
          <table>
            ...
            <tbody id="dash-tx-tbody">${renderTxRows(txs)}</tbody>
          </table>
  ...
  // ── Lazy-load: Epoch Trends (GraphQL epochs root) ──
  fetchDashboardEpochTrends().then((trendData) => {
  ...
  // ── Lazy-load: Stablecoin Supply ──
  fetchStablecoinSupply().then(data => {
    const el = document.getElementById("stablecoin-card");
    if (!el) return;
    if (!data || !data.coins.length) { el.querySelector(".card-body").innerHTML = '<div class="empty">Supply data unavailable</div>'; return; }
    el.querySelector(".card-body").innerHTML = `
  ...
  // ── Lazy-load: DeFi TVL Breakdown + Protocol Rankings + stat boxes ──
  fetchEcosystemStats().then(stats => {
    // Populate TVL stat box
  ...
  // ── Auto-refresh every 5s (uses data-stat selectors + element IDs) ──
  if (dashboardTimer) clearInterval(dashboardTimer);
  dashboardTimer = setInterval(async () => {
    if (parseRoute(getRoute()).page !== "home") { clearInterval(dashboardTimer); dashboardTimer = null; return; }
    try {
      const fresh = await gql(`{
        checkpoint { sequenceNumber digest timestamp networkTotalTransactions }
        transactions(last: 10, filter: {}) {
          nodes {
            digest
            sender { address }
      ...
      const fcp = fresh.checkpoint;
      const ftxs = fresh.transactions.nodes.reverse();
      const fcps = fresh.checkpoints.nodes.reverse();
      // Update stat boxes by data-stat attribute
      const cpEl = document.querySelector('[data-stat="checkpoint"]');
      if (cpEl) cpEl.textContent = fmtNumber(fcp.sequenceNumber);
      const txEl = document.querySelector('[data-stat="total-txns"]');
      if (txEl) txEl.textContent = fmtNumber(fcp.networkTotalTransactions);
      const cpTimeEl = document.querySelector('[data-stat-sub="cp-time"]');
      if (cpTimeEl) { cpTimeEl.textContent = timeAgo(fcp.timestamp); cpTimeEl.title = fmtTime(fcp.timestamp); }
      // Update tables by ID using enriched helpers
      const cpBody = document.getElementById("dash-cp-tbody");
      if (cpBody) cpBody.innerHTML = renderCpRows(fcps);
      const txBody = document.getElementById("dash-tx-tbody");
      if (txBody) txBody.innerHTML = renderTxRows(ftxs);
...
async function renderCheckpoints(app, after = null) {
  ...
  const cps = data.checkpoints.nodes.reverse();
  const pi = data.checkpoints.pageInfo;
  ...
  app.innerHTML = `
    ...
    <div class="card">
      <div class="card-body">
        <table>
          <thead><tr>
            <th>Checkpoint</th><th>Digest</th><th>Timestamp</th><th>Gas (Computation)</th>
          </tr></thead>
          <tbody>
            ${cps.map(c => `<tr>
              <td><a class="hash-link" href="#/checkpoint/${c.sequenceNumber}">${fmtNumber(c.sequenceNumber)}</a></td>
              ...
              <td>${fmtSui(c.rollingGasSummary?.computationCost)}</td>
            </tr>`).join("")}
          </tbody>
        </table>
...
async function renderTxDetail(app, digest) {
  ...
  app.innerHTML = `
    ...
    ${isPTB && inputs.length ? section("sec-inputs", "Inputs", `${inputs.length}${inputsTruncated ? "+" : ""}`, false, renderInputs(), { truncated: inputsTruncated }) : ""}
    ${section("sec-balances", "Balance Changes", `${balances.length}${balancesTruncated ? "+" : ""}`, true, renderBalances(), { truncated: balancesTruncated })}
...
async function fetchStablecoinSupply() {
  ...
  try {
    ...
    // Strategy 1: coinMetadata.supply (batched, max 3 per request to stay under query limit)
    for (let i = 0; i < STABLECOINS_METADATA.length; i += 3) {
      const batch = STABLECOINS_METADATA.slice(i, i + 3);
      const aliases = batch.map((s, j) => `s${j}: coinMetadata(coinType: "${s.type}") { supply }`);
      const data = await gql(`{ ${aliases.join("\n")} }`);
      batch.forEach((s, j) => {
        const raw = data?.[`s${j}`]?.supply;
        if (raw) coins.push({ symbol: s.symbol, supply: Number(raw) / Math.pow(10, s.decimals), color: s.color });
      });
    }

    // Strategy 2: Wormhole token_registry dynamic field (wrapped TreasuryCaps)
    for (const wh of STABLECOINS_WORMHOLE) {
      try {
        const keyType = `${WORMHOLE_KEY_PKG}::token_registry::Key<${wh.type}>`;
        const data = await gql(`{ address(address: "${WORMHOLE_REGISTRY}") { dynamicField(name: { type: "${keyType}", bcs: "AA==" }) { value { ... on MoveValue { json } } } } }`);
        const j = data?.address?.dynamicField?.value?.json;
        if (j?.treasury_cap?.total_supply?.value) {
          const supply = Number(j.treasury_cap.total_supply.value) / Math.pow(10, j.decimals || wh.decimals);
          coins.push({ symbol: wh.symbol, supply, color: wh.color });
        }
      } catch (_) { /* skip failed wormhole lookups */ }
    }
    ...
    for (const p of STABLECOINS_PROTOCOL) {
      try {
        ...
        if (json) {
          ...
          if (val) coins.push({ symbol: p.symbol, supply: Number(val) / Math.pow(10, p.decimals), color: p.color });
        }
      } catch (_) { /* skip failed protocol lookups */ }
    }
...
async function fetchDeterministicDefiWindowSample(windowKeyOrForce = DEFI_WINDOW_DEFAULT_KEY, forceMaybe = false) {
  ...
  return withTimedCache(cacheState, DEFI_WINDOW_SAMPLE_TTL_MS, force, async () => {
    ...
    let before = null;
    let callsUsed = 1; // latest checkpoint query
...
async function fetchPackageUpgradeSnapshot(pkgAddr, force = false) {
  ...
  return withTimedCache(packageDetailCache[pkg], PACKAGE_DETAIL_TTL_MS, force, async () => {
    ...
    await Promise.all(versionAddresses.map(async (addr) => {
      try {
        const mData = await gql(`query($addr: SuiAddress!) {
          object(address: $addr) { asMovePackage { modules(first: 250) { nodes { name } } } }
        }`, { addr });
        modulesByAddress[addr] = (mData?.object?.asMovePackage?.modules?.nodes || [])
          .map(m => m?.name)
          .filter(Boolean);
      } catch (e) {
...
async function fetchDefiRiskSnapshot(windowKeyOrForce = DEFI_WINDOW_DEFAULT_KEY, forceMaybe = false) {
  const { windowKey, force } = parseDefiWindowAndForce(windowKeyOrForce, forceMaybe);
  const cacheState = getKeyedCacheState(defiRiskCacheByWindow, windowKey);
  return withTimedCache(cacheState, DEFI_RISK_TTL_MS, force, async () => {
    const [lending, stable, flows] = await Promise.all([
      fetchLendingRatesOverview(force),
      fetchDefiStablecoinSnapshot(windowKey, force),
      fetchDefiFlowSnapshot(windowKey, force),
    ]);
    const alerts = [];
    const allLendingRows = [...(lending?.byToken?.SUI || []), ...(lending?.byToken?.USDC || [])];
    const lendingRows = allLendingRows
      .filter(r => Number.isFinite(r.utilization) && Number.isFinite(r.borrowBps));
    for (const r of lendingRows) {
      const spreadBps = Number.isFinite(r.supplyBps) ? (r.borrowBps - r.supplyBps) : null;
      if (r.utilization > 0.9) alerts.push({ severity: "high", area: "Lending", protocol: r.protocol, token: r.token, metric: "Utilization", value: `${(r.utilization * 100).toFixed(1)}%`, note: "Very high utilization" });
      else if (r.utilization > 0.8) alerts.push({ severity: "medium", area: "Lending", protocol: r.protocol, token: r.token, metric: "Utilization", value: `${(r.utilization * 100).toFixed(1)}%`, note: "Elevated utilization" });
      if (r.borrowBps > 2500) alerts.push({ severity: "high", area: "Lending", protocol: r.protocol, token: r.token, metric: "Borrow APR", value: `${(r.borrowBps / 100).toFixed(2)}%`, note: "Borrow cost spike" });
      else if (r.borrowBps > 1600) alerts.push({ severity: "medium", area: "Lending", protocol: r.protocol, token: r.token, metric: "Borrow APR", value: `${(r.borrowBps / 100).toFixed(2)}%`, note: "Borrow cost elevated" });
      if (spreadBps != null && spreadBps > 1500) alerts.push({ severity: "medium", area: "Lending", protocol: r.protocol, token: r.token, metric: "Spread", value: `${(spreadBps / 100).toFixed(2)}%`, note: "Wide borrow/supply spread" });
    }
    const topStable = (stable?.coins || [])[0];
    // Concentration is tracked as context but does not contribute to critical risk alerts.
    const largeFlows = (flows?.rows || []).filter(r => r.usdValue > 500000);
    if (largeFlows.length >= 8) alerts.push({ severity: "medium", area: "Flows", protocol: "Cross-Protocol", token: "USD", metric: "Large Flows", value: `${largeFlows.length}`, note: "High whale flow count in selected window" });

    const score = Math.min(100, alerts.reduce((s, a) => s + (a.severity === "high" ? 18 : a.severity === "medium" ? 10 : 4), 0));
    const highCount = alerts.filter(a => a.severity === "high").length;
    const mediumCount = alerts.filter(a => a.severity === "medium").length;
    const lendingUnavailable = allLendingRows.filter(r => r.error).length;
    const lowConfidenceFlowRows = flows?.coverage?.lowConfidenceRows || 0;
    const totalFlowRows = flows?.coverage?.flowRows || 0;
    const signals = [
      highCount
        ? `${highCount} high-severity threshold alerts are active.`
        : "No high-severity threshold alerts are active.",
      topStable
        ? `Top stablecoin share is ${topStable.pct.toFixed(1)}% (${topStable.symbol}) and is tracked as a non-critical concentration watchlist metric.`
        : "Stablecoin concentration signal unavailable.",
      totalFlowRows
        ? `${((lowConfidenceFlowRows / totalFlowRows) * 100).toFixed(1)}% of flow rows feeding risk are low-confidence protocol mappings.`
        : "Flow confidence coverage unavailable for this risk snapshot.",
    ];
    return {
      fetchedAt: new Date().toISOString(),
      score,
      ...
      highCount,
      mediumCount,
      lendingRows: lendingRows.sort((a, b) => (b.utilization || 0) - (a.utilization || 0)),
      topStable: topStable || null,
      largeFlowCount: largeFlows.length,
      coverage: {
        lendingRowsTotal: allLendingRows.length,
        lendingRowsLive: lendingRows.length,
        lendingRowsUnavailable: lendingUnavailable,
        stablecoinsTracked: stable?.coins?.length || 0,
        flowRows: totalFlowRows,
        lowConfidenceFlowRows,
      },
      signals,
      window: flows?.window || stable?.window || { windowKey },
    };
  });
}
...
async function fetchDefiWalletBalances(addr) {
  ...
  return allBalances
    .filter(b => Number(b.rawBalance) > 0)
    .map(b => {
...
async function fetchSuilendPositions(addr) {
  ...
  for (let i = 0; i < obligationIds.length; i++) {
    ...
    const deposits = (ob.deposits || []).map(d => {
      ...
      return { symbol: sym, amountUsd: Number(d.market_value?.value || 0) / 1e18 };
    });
    const borrows = (ob.borrows || []).map(b => {
      const sym = (b.coin_type?.name || "").split("::").pop();
      return { symbol: sym, amountUsd: Number(b.market_value?.value || 0) / 1e18 };
    });
...
async function fetchNaviPositions(addr) {
  // Step 1: Look up user info via dynamic field
  ...
  // Step 3: Batch-fetch user balances from supply/borrow tables
  const balQueries = [];
  ...
  for (const b of balResults) {
    ...
    if (b.type === "supply") {
      ...
      totalDepUsd += usd;
      deposits.push({ symbol: sym, amountUsd: usd, ltv: Number(rv.ltv || 0) / NAVI_RAY });
    } else {
      const idx = Number(rv.current_borrow_index || 1e27) / NAVI_RAY;
      const human = (scaled * idx) / Math.pow(10, dec);
      const usd = human * (defiPrices[sym] || 0);
      totalBorUsd += usd;
      borrows.push({ symbol: sym, amountUsd: usd });
    }
  }
...
async function fetchAlphaPositions(addr) {
  ...
  for (let i = 0; i < posIds.length; i++) {
    ...
    const deps = collContents.map(c => { const m = Number(c.key || 0); return { symbol: ALPHA_MARKETS[m] || `Market${m}`, amountUsd: collContents.length > 0 ? depositedUsd / collContents.length : 0 }; });
    const loans = (data.loans || []).map(l => { const sym = String(l.coin_type?.name || "").split("::").pop() || "?"; const dec = getDecimals(sym); const usd = (Number(l.amount || 0) / Math.pow(10, dec)) * (defiPrices[sym] || 0); return { symbol: sym, amountUsd: usd || (borrowedUsd / Math.max((data.loans || []).length, 1)) }; });
    positions.push({ protocol: "Alpha", deposits: deps, borrows: loans, depositedUsd, borrowedUsd, healthFactor: borrowedUsd > 0 ? safeCollUsd / borrowedUsd : Infinity, netUsd: depositedUsd - borrowedUsd });
  }
  return positions;
}
...
async function fetchScallopPositions(addr) {
  ...
  for (const info of obInfos) {
    ...
    for (const f of collNodes) {
      ...
      totalDepUsd += usd;
      deposits.push({ symbol: resolved.symbol, amountUsd: usd });
    }
    for (const f of debtNodes) {
      ...
      const usd = human * (defiPrices[resolved.symbol] || 0);
      totalBorUsd += usd;
      borrows.push({ symbol: resolved.symbol, amountUsd: usd });
    }
...
async function renderAddress(app, addr) {
  ...
  if (!data.address) {
    return renderObjectDetail(app, addr);
  }
  ...
  // Prefetch coin metadata for balances
  await prefetchCoinMeta(balances.map(b => b.coinType?.repr).filter(Boolean));

  // DeFi data (loaded eagerly since it's the default tab)
  ...
  async function loadDefi() {
    ...
    if (regularBals.length) {
      ...
      for (const b of regularBals.slice(0, 15)) {
        ...
        const supplyFmt = supplyRaw != null ? fmtCompact(supplyRaw / Math.pow(10, meta?.decimals || 9)) : "—";
        html += `<tr><td class="u-mono-12">${b.symbol}</td><td class="u-mono-12">${b.amount >= 0.01 ? b.amount.toLocaleString(undefined, {maximumFractionDigits:4}) : b.amount.toFixed(8)}</td><td style="font-family:var(--mono);font-size:12px;color:var(--green)">${b.usdValue > 0 ? fmtUsdFromFloat(b.usdValue) : "—"}</td><td class="u-mono-11-dim">${supplyFmt}</td></tr>`;
      }
    ...
    if (lstBals.length) {
      ...
      for (const b of lstBals) {
        ...
        html += `<tr>
          ...
          <td class="u-mono-12">${b.amount >= 0.01 ? b.amount.toLocaleString(undefined, {maximumFractionDigits:4}) : b.amount.toFixed(8)}</td>
          <td style="font-family:var(--mono);font-size:12px;color:var(--text-dim)">${b.suiEquiv > 0 ? b.suiEquiv.toLocaleString(undefined, {maximumFractionDigits:4}) + " SUI" : "—"}<span class="u-fs10-dim">${rateLabel}</span></td>
          <td style="font-family:var(--mono);font-size:12px;color:var(--accent)">${b.usdValue > 0 ? fmtUsdFromFloat(b.usdValue) : "—"}</td>
        </tr>`;
      }
      html += `</tbody></table>`;
    }
    ...
    if (allLending.length) {
      ...
      for (const pos of allLending) {
        ...
        if (pos.deposits?.length) {
          html += `<div style="font-size:12px;color:var(--text-dim);margin-bottom:4px">Deposits</div>`;
          for (const d of pos.deposits) html += `<div class="u-row-between-sm"><span>${d.symbol}</span><span class="u-c-green">${d.amountUsd > 0 ? fmtUsdFromFloat(d.amountUsd) : "—"}</span></div>`;
        }
        if (pos.borrows?.length) {
          html += `<div style="font-size:12px;color:var(--text-dim);margin:6px 0 4px">Borrows</div>`;
          for (const b of pos.borrows) html += `<div class="u-row-between-sm"><span>${b.symbol}</span><span class="u-c-red">${b.amountUsd > 0 ? fmtUsdFromFloat(b.amountUsd) : "—"}</span></div>`;
        }
    ...
    if (allDexLp.length) {
      ...
      for (const pos of allDexLp) {
        ...
        html += `<div style="display:flex;gap:20px;font-size:13px"><span>${pos.amountA.toFixed(4)} ${pos.symbolA} <span class="u-c-dim">${pos.usdA > 0 ? fmtUsdFromFloat(pos.usdA) : ""}</span></span><span>${pos.amountB.toFixed(4)} ${pos.symbolB} <span class="u-c-dim">${pos.usdB > 0 ? fmtUsdFromFloat(pos.usdB) : ""}</span></span></div>`;
    ...
    if (db.positions.length) {
      ...
      for (const pos of db.positions) {
        ...
        for (const c of pos.collateral) {
          ...
          totalCollUsd += usd;
          html += `<div class="u-row-between-sm"><span>${c.symbol}</span><span class="u-c-green">${human.toLocaleString(undefined, {maximumFractionDigits:4})} ${usd > 0 ? fmtUsdFromFloat(usd) : ""}</span></div>`;
        }
        let totalDebtUsd = 0;
        if (baseBorrowed > 0 || quoteBorrowed > 0) {
          ...
          if (baseBorrowed > 0) {
            ...
            const usd = human * (defiPrices[sym] || 0);
            totalDebtUsd += usd;
            html += `<div class="u-row-between-sm"><span>${sym}</span><span class="u-c-red">${human.toLocaleString(undefined, {maximumFractionDigits:4})} ${usd > 0 ? fmtUsdFromFloat(usd) : ""}</span></div>`;
          }
          if (quoteBorrowed > 0) {
  ...
  function renderTabs(active) {
    ...
    if (active === "objects") {
      ...
      if (btn) btn.onclick = async () => {
        ...
        }`, { addr, after: objPageInfo.endCursor });
  ...
  app.innerHTML = `
    <div class="page-title">
      ...
      ${copyLinkBtn()}${viewQueryBtn('address_detail', { addr })}
    </div>
    <div class="card u-mb16">
      <div class="card-body">
        <div class="detail-row">
...
async function renderObjectDetail(app, id) {
  ...
  if (!obj) {
    await renderDeletedObjectDetail(app, id);
    return;
  }
  ...
  app.innerHTML = `
    ...
    <div class="card u-mb16">
      <div class="card-body">
        <div class="detail-row">
          <div class="detail-key">Object ID</div>
          <div class="detail-val">${obj.address} ${copyBtn(obj.address)}</div>
        </div>
        ...
        <div class="detail-row">
          <div class="detail-key">Digest</div>
          <div class="detail-val">${obj.digest || "—"}</div>
        </div>
        <div class="detail-row">
...
// ── Validators / Staking ────────────────────────────────────────────────
async function renderValidators(app) {
  ...
  let allValNodes = [];
  let cursor = null;
  let epochId = 0, refGas = "0", startTs = "";
  ...
  const validators = allValNodes.map(v => {
          ...
          <tbody>
            ${validators.map((v, i) => {
              ...
              return `<tr style="cursor:pointer" data-action="validator-open-detail" data-idx="${i}">
                ...
                <td class="u-ta-right-mono">${fmtCompact(stakeSui)}${pendingNet !== 0 ? `<div style="font-size:10px;color:${pendingNet > 0 ? 'var(--green)' : 'var(--red)'}">${pendingNet > 0 ? '+' : ''}${fmtCompact(pendingNet / 1e9)}</div>` : ""}</td>
                <td style="text-align:right;font-family:var(--mono);font-size:12px;color:var(--text-dim)">${fmtCompact(rewardsSui)}</td>
                <td class="u-ta-right-mono">${votingPct}%</td>
                <td class="u-ta-right-mono">${(v.commission / 100).toFixed(1)}%</td>
                <td style="text-align:right;font-family:var(--mono);color:var(--green)">${estApy.toFixed(2)}%</td>
...
async function renderEvents(app) {
  ...
  async function loadEvents() {
    const filters = [];
    if (eventTypeFilter) filters.push(`type: "${eventTypeFilter}"`);
    if (senderFilter) filters.push(`sender: "${senderFilter}"`);
    const filterArg = filters.length ? `filter: { ${filters.join(", ")} }` : "";
    const data = await gql(`{ events(last: 50 ${filterArg}) { nodes { contents { type { repr } json } sender { address } timestamp transactionModule { name package { address } } } } }`);
  ...
  function renderEventsContent() {
    ...
    html += renderDefiScopeBar({
      sampleLabel: `Last ${fmtNumber(eventsData.length)} events`,
      fetchedAt,
    ...
    html += `<div class="stats-grid u-mb16">
      ...
      <div class="stat-box"><div class="stat-label">Unique Packages</div><div class="stat-value">${stats.uniquePkgs.length}</div></div>
    ...
    if (!eventsData.length) {
      ...
      html += `<table><thead><tr><th>Event Type</th><th>Package</th><th>Module</th><th>Sender</th><th>Data</th><th>Time</th></tr></thead><tbody>`;
      eventsData.forEach((ev, idx) => {
        const typeRepr = ev.contents?.type?.repr || "—";
        const pkg = ev.transactionModule?.package?.address;
        const modName = ev.transactionModule?.name || "—";
        const pkgMvr = pkg && mvrNameCache[pkg] ? `@${mvrNameCache[pkg]}` : null;
        const sender = ev.sender?.address;
        ...
        // Event type with MVR name applied
        ...
        html += `<tr>
          ...
          <td class="u-fs12">${pkg ? hashLink(pkgMvr || truncHash(pkg), '/object/' + pkg) : "—"}</td>
          <td class="u-fs12-dim">${modName}</td>
          <td>${sender ? hashLink(sender, '/address/' + sender) : "—"}</td>
          <td>${hasJson ? `<button data-action="events-toggle-json" data-row-id="${rowId}" aria-expanded="false" style="background:var(--surface2);border:1px solid var(--border);border-radius:4px;color:var(--accent);font-size:11px;padding:2px 10px;cursor:pointer;font-family:var(--mono)">{${keyCount}} fields</button>` : '<span class="u-c-dim">—</span>'}</td>
          <td style="white-space:nowrap">${timeTag(ev.timestamp)}</td>
        </tr>`;
        if (hasJson) {
          html += `<tr id="${rowId}" style="display:none"><td colspan="6" style="padding:0 8px 8px">${jsonTreeBlock(ev.contents.json, 280)}</td></tr>`;
        }
      });
      html += `</tbody></table>`;
    }
...
function fmtHistoryMetricValue(metric, v) {
  ...
  if (metric === "network") return fmtNumber(Math.round(v));
  ...
  function renderContent() {
    ...
    return `
      ...
      <div class="card u-mb16">
        ...
        <div class="card-body">
          ${rows.length ? `<table>
            <thead><tr><th>Package</th><th>Source</th><th>Category</th><th>Conf.</th><th class="u-ta-right">Txs</th><th class="u-ta-right">Calls</th><th class="u-ta-right">Success</th><th class="u-ta-right">Senders</th><th>Latest</th></tr></thead>
            <tbody>
              ${rows.map(r => {
    ...
    return `
      ...
      <div class="card">
        ...
        <div class="card-body" style="padding:12px 16px;display:flex;flex-wrap:wrap;gap:8px">
          ...
          <a href="#/defi-flows" class="hash-link u-panel-sm">Flows</a>
          <a href="#/defi-risk" class="hash-link u-panel-sm">Risk Monitor</a>
        </div>
      </div>
    `;
  }
...
async function renderDefiFlows(app) {
  ...
  function renderContent() {
    ...
    return `
      <div class="page-title">DeFi Flows <span class="type-tag">Selected Window</span></div>
      ${scope}
      <div class="stats-grid">
...
// ── DeFi Risk Monitor ────────────────────────────────────────────────
async function renderDefiRisk(app) {
  const routeParams = splitRouteAndParams(getRoute()).params;
  let data = await fetchDefiRiskSnapshot(DEFI_WINDOW_DEFAULT_KEY, false);
  let severityFilter = ["all", "high", "medium"].includes(routeParams.get("sev")) ? routeParams.get("sev") : "all";
  let areaFilter = ["all", "lending", "stablecoins", "flows"].includes(routeParams.get("area")) ? routeParams.get("area") : "all";
  let lendingSort = ["util", "borrow", "spread"].includes(routeParams.get("lsort")) ? routeParams.get("lsort") : "util";

  function persistDefiRiskState() {
    setRouteParams({
      sev: severityFilter !== "all" ? severityFilter : null,
      area: areaFilter !== "all" ? areaFilter : null,
      lsort: lendingSort !== "util" ? lendingSort : null,
    });
  }
  ...
  function filteredAlerts() {
    ...
    if (areaFilter !== "all") rows = rows.filter(a => String(a.area || "").toLowerCase() === areaFilter.toLowerCase());
    rows.sort((a, b) => (a.severity === b.severity ? 0 : (a.severity === "high" ? -1 : 1)));
    return rows;
  }

  function sortedLendingRows() {
    let rows = [...(data.lendingRows || [])];
    if (lendingSort === "borrow") rows.sort((a, b) => (b.borrowBps || 0) - (a.borrowBps || 0));
    else if (lendingSort === "spread") rows.sort((a, b) => ((b.borrowBps || 0) - (b.supplyBps || 0)) - ((a.borrowBps || 0) - (a.supplyBps || 0)));
    else rows.sort((a, b) => (b.utilization || 0) - (a.utilization || 0));
    return rows;
  }

  function renderContent() {
    const alerts = filteredAlerts();
    const lendingRows = sortedLendingRows();
    const coverage = data.coverage || {};
    const scoreColor = data.score >= 70 ? "var(--red)" : data.score >= 40 ? "var(--yellow)" : "var(--green)";
    const flowRows = coverage.flowRows || 0;
    const lowFlowShare = flowRows ? ((coverage.lowConfidenceFlowRows || 0) / flowRows * 100) : 0;
    const scope = renderDefiScopeBar({
      sampleLabel: "Threshold rules over lending + stablecoin + flow snapshots",
      fetchedAt: data.fetchedAt,
      ttlMs: DEFI_RISK_TTL_MS,
      refreshAction: "defi-risk-refresh",
      leftControls: `
        <span class="u-fs12-dim">Severity</span>
        <select data-action="defi-risk-severity" class="ui-control">
        ...
        <span class="u-fs12-dim">Area</span>
        <select data-action="defi-risk-area" class="ui-control">
          <option value="all" ${areaFilter === "all" ? "selected" : ""}>All</option>
          <option value="lending" ${areaFilter === "lending" ? "selected" : ""}>Lending</option>
          <option value="stablecoins" ${areaFilter === "stablecoins" ? "selected" : ""}>Stablecoins</option>
          <option value="flows" ${areaFilter === "flows" ? "selected" : ""}>Flows</option>
        </select>
        <span class="u-fs12-dim">Lending Sort</span>
        <select data-action="defi-risk-lending-sort" class="ui-control">
          <option value="util" ${lendingSort === "util" ? "selected" : ""}>Utilization</option>
          <option value="borrow" ${lendingSort === "borrow" ? "selected" : ""}>Borrow APR</option>
          <option value="spread" ${lendingSort === "spread" ? "selected" : ""}>Spread</option>
        </select>
      `,
    });
    const methods = renderDefiMethodCard([
      "Alerts are deterministic threshold checks on lending utilization/APR and large-flow counts.",
      "Stablecoin concentration is shown as context but does not escalate critical alert severity.",
      "No predictive model is used; score is a weighted sum of active rule breaches in this snapshot.",
      "Lending inputs come from protocol state objects and utilization-derived APR computations.",
      "Coverage reports unavailable lending rows and low-confidence flow mappings used by rules.",
    ], "How Rules Work");
    const signals = renderDefiSignalsCard(data.signals || []);

    return `
      <div class="page-title">DeFi Risk Monitor <span class="type-tag">Threshold Rules</span></div>
      ${scope}
      <div class="stats-grid">
        <div class="stat-box"><div class="stat-label">Rule Score</div><div class="stat-value" style="color:${scoreColor}">${data.score}</div><div class="stat-sub">0 = calm · 100 = stressed</div></div>
        <div class="stat-box"><div class="stat-label">High Alerts</div><div class="stat-value u-c-red">${data.highCount}</div><div class="stat-sub">requires attention</div></div>
        <div class="stat-box"><div class="stat-label">Medium Alerts</div><div class="stat-value u-c-yellow">${data.mediumCount}</div><div class="stat-sub">watchlist</div></div>
        <div class="stat-box"><div class="stat-label">Top Stablecoin Share</div><div class="stat-value">${data.topStable ? data.topStable.pct.toFixed(1) + "%" : "—"}</div><div class="stat-sub">${data.topStable ? data.topStable.symbol : "—"}</div></div>
        <div class="stat-box"><div class="stat-label">High Util Markets</div><div class="stat-value">${lendingRows.filter(r => (r.utilization || 0) > 0.85).length}</div><div class="stat-sub">utilization &gt; 85%</div></div>
        <div class="stat-box"><div class="stat-label">Large Flow Count</div><div class="stat-value">${data.largeFlowCount}</div><div class="stat-sub">rows above $500k</div></div>
        <div class="stat-box"><div class="stat-label">Unavailable Lending Rows</div><div class="stat-value">${fmtNumber(coverage.lendingRowsUnavailable || 0)}</div><div class="stat-sub">of ${fmtNumber(coverage.lendingRowsTotal || 0)} total rows</div></div>
        <div class="stat-box"><div class="stat-label">Low-Confidence Flow Inputs</div><div class="stat-value" style="color:${lowFlowShare > 25 ? "var(--yellow)" : "var(--text)"}">${fmtNumber(coverage.lowConfidenceFlowRows || 0)}</div><div class="stat-sub">${lowFlowShare.toFixed(1)}% of flow rows</div></div>
      </div>
      ...
      <div class="card u-mb16">
        ...
        <div class="card-body">
          ${alerts.length ? `<table>
            <thead><tr><th>Severity</th><th>Area</th><th>Protocol</th><th>Token</th><th>Metric</th><th class="u-ta-right">Value</th><th>Note</th></tr></thead>
            <tbody>
              ${alerts.map(a => `<tr>
                <td>${defiSeverityBadge(a.severity)}</td>
                <td>${a.area}</td>
                <td>${escapeHtml(a.protocol)}</td>
                <td>${a.token}</td>
                <td>${a.metric}</td>
                <td class="u-ta-right-mono">${a.value}</td>
                <td class="u-fs12-dim">${escapeHtml(a.note)}</td>
              </tr>`).join("")}
            </tbody>
          </table>` : renderEmpty("No active risk alerts in the current filtered view.")}
        </div>
      </div>

      <div class="card">
        <div class="card-header">Lending Heatmap Inputs</div>
        <div class="card-body">
          ${lendingRows.length ? `<table>
            <thead><tr><th>Protocol</th><th>Token</th><th class="u-ta-right">Supply APR</th><th class="u-ta-right">Borrow APR</th><th class="u-ta-right">Utilization</th><th class="u-ta-right">Spread</th></tr></thead>
            <tbody>
              ${lendingRows.map(r => {
                const util = (r.utilization || 0) * 100;
                const utilColor = util > 90 ? "var(--red)" : util > 80 ? "var(--yellow)" : "var(--green)";
                const spreadBps = Number.isFinite(r.supplyBps) ? (r.borrowBps - r.supplyBps) : null;
                return `<tr>
                  <td class="u-fw-600">${escapeHtml(r.protocol)}</td>
                  <td>${r.token}</td>
                  <td class="u-ta-right-mono">${Number.isFinite(r.supplyBps) ? (r.supplyBps / 100).toFixed(2) + "%" : "—"}</td>
                  <td class="u-ta-right-mono">${Number.isFinite(r.borrowBps) ? (r.borrowBps / 100).toFixed(2) + "%" : "—"}</td>
                  <td style="text-align:right;font-family:var(--mono);color:${utilColor}">${util.toFixed(1)}%</td>
                  <td class="u-ta-right-mono">${spreadBps != null ? (spreadBps / 100).toFixed(2) + "%" : "—"}</td>
                </tr>`;
              }).join("")}
            </tbody>
          </table>` : renderEmpty("No lending rows available.")}
        </div>
      </div>
    `;
  }

  const setDefiRiskSeverity = (v) => {
    severityFilter = ["all", "high", "medium"].includes(v) ? v : "all";
    persistDefiRiskState();
    app.innerHTML = renderContent();
  };
  const setDefiRiskArea = (v) => {
    areaFilter = ["all", "lending", "stablecoins", "flows"].includes(v) ? v : "all";
    persistDefiRiskState();
    app.innerHTML = renderContent();
  };
  const setDefiRiskLendingSort = (v) => {
    lendingSort = ["util", "borrow", "spread"].includes(v) ? v : "util";
    persistDefiRiskState();
    app.innerHTML = renderContent();
  };
  const refreshDefiRisk = async () => {
    app.innerHTML = renderLoading();
    data = await fetchDefiRiskSnapshot(DEFI_WINDOW_DEFAULT_KEY, true);
    app.innerHTML = renderContent();
  };
  if (app._defiRiskChangeHandler) app.removeEventListener("change", app._defiRiskChangeHandler);
  app._defiRiskChangeHandler = (ev) => {
    const target = ev.target;
    if (!(target instanceof Element)) return;
    const action = target.getAttribute("data-action");
    if (!action) return;
    if (action === "defi-risk-severity") {
      setDefiRiskSeverity(target.value);
      return;
    }
    if (action === "defi-risk-area") {
      setDefiRiskArea(target.value);
      return;
    }
    if (action === "defi-risk-lending-sort") {
      setDefiRiskLendingSort(target.value);
    }
  };
  app.addEventListener("change", app._defiRiskChangeHandler);
  if (app._defiRiskClickHandler) app.removeEventListener("click", app._defiRiskClickHandler);
  app._defiRiskClickHandler = async (ev) => {
    const trigger = ev.target?.closest?.("[data-action]");
    if (!trigger || !app.contains(trigger)) return;
    if (trigger.getAttribute("data-action") !== "defi-risk-refresh") return;
    ev.preventDefault();
    await refreshDefiRisk();
  };
  app.addEventListener("click", app._defiRiskClickHandler);
  app.innerHTML = renderContent();
}
...
async function renderDefiRates(app) {
  ...
  async function load(force = false) {
    try {
      ...
      data = null;
      loadErr = e?.message || "Failed to load lending rates.";
    }
  }
...
async function renderProtocolConfig(app) {
  ...
  function renderContent() {
    ...
    html += `<div class="stats-grid u-mb16">
      <div class="stat-box"><div class="stat-label">Chain Identifier</div><div class="stat-value u-fs16">${chainIdentifier ? escapeHtml(chainIdentifier) : "—"}</div><div class="stat-sub">GraphQL root: chainIdentifier</div></div>
    ...
    <div class="card u-mb16">
      ...
      <div class="card-body" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:10px">
        ...
        <a href="#/defi-overview" class="hash-link u-panel-block">
          ...
          <div class="u-fs12-dim">DeFi Overview -> Rates / DEX / Flows / Risk</div>
        </a>
