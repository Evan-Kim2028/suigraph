...
async function discoverPoolAddresses(coinTypes) {
  ...
  for (const ct of needed) {
    ...
    totalPools += poolAddressCache[ct].pools.length;
  }
  console.log(`[pool-oracle] discovery: ${needed.length} tokens, ${aliases.length} aliases, ${totalPools} pools found`);
}
...
async function readPoolPrices(coinTypesBySymbol) {
  ...
  console.log(`[pool-oracle] readPoolPrices: ${addresses.length} addresses, poolById keys: ${Object.keys(poolById).length}`);
  for (const addr of addresses) {
    const obj = poolById[addr];
    if (!obj) { console.log(`[pool-oracle] miss for ${addr.slice(0,16)}...`); continue; }
    ...
    if (!json || !typeRepr) { console.log(`[pool-oracle] no json/type for ${addr.slice(0,16)}...`); continue; }
...
async function fetchPoolOraclePrices(specificCoinTypes) {
  ...
  console.log("[pool-oracle] prices set:", Object.keys(prices).length, "tokens:", Object.entries(prices).map(([s,p]) => `${s}=$${p.toFixed(4)}`).join(", "));
...
async function fetchDefiPrices(force = false, { skipOracle = false } = {}) {
  ...
  defiPricesInFlight = (async () => {
    ...
    // BTC-pegged: use xBTC DeepBook spot price for all BTC variants
    if (defiPrices[BTC_PRICE_SOURCE]) {
      for (const sym of BTC_PEGGED_SYMBOLS) defiPrices[sym] = defiPrices[BTC_PRICE_SOURCE];
    }
...
function isCoreToken(sym) { return CORE_LENDING_TOKENS.has(sym) || sym.toUpperCase().includes("BTC"); }
...
async function renderDefiRates(app) {
  ...
  function sortedRows(allRows) {
    ...
    else if (sortKey === "tvl") rows.sort((a, b) => ((b.totalSupplyHuman || 0) * (defiPrices[b.token] || 0)) - ((a.totalSupplyHuman || 0) * (defiPrices[a.token] || 0)));
    else if (sortKey === "borrowed") rows.sort((a, b) => ((b.totalBorrowHuman || 0) * (defiPrices[b.token] || 0)) - ((a.totalBorrowHuman || 0) * (defiPrices[a.token] || 0)));
