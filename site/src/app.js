...
async function renderCoin(app, routeCoinType = "") {
  ...
  try {
    ...
    const PAGE_SIZE = 20;
    const GLOBAL_SCAN_MAX_TX = 80;
    const GLOBAL_SCAN_EMPTY_PAGE_THRESHOLD = 2;
    const OBJECT_SCAN_MAX_PAGES = 10;
    const OBJECT_SCAN_MAX_DIGESTS = 320;
    const OBJECT_TX_LOAD_LIMIT = 80;
    let scannedTx = 0;
    let scannedPages = 0;
    let before = null;
    ...
    function ingestCoinTransaction(tx) {
      ...
      const txAction = classifyTransactionAction(tx);
      const txEventActionLabel = String(txAction?.label || "");
