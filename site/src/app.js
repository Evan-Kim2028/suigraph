...
function classifyCoinBalanceFlowKind(sentRaw, recvRaw, eventActionLabel = "") {
...
async function renderCoin(app, routeCoinType = "") {
  ...
  async function fetchCoinObjectDigestCandidates(coinTypeValue, { maxPages = 8, maxDigests = 280 } = {}) {
  ...
  try {
    ...
    const EARLY_EMPTY_PAGE_THRESHOLD = 4;
    let scannedTx = 0;
    let scannedPages = 0;
    let before = null;
    let hasPreviousPage = true;
    let stoppedForTargetedFallback = false;
    ...
    function ingestCoinTransaction(tx) {
      ...
      const txEventAction = classifyEventAction(txEvents);
      const txEventActionLabel = String(txEventAction?.label || "");
      ...
      if (coinBalanceRows.length) {
        ...
        if (amountRaw > 0n) {
          const kind = classifyCoinFlowKindWithContext(eff, targetKey, sentRaw, recvRaw, txEventActionLabel);
          transferRows.push({
            digest: tx?.digest || "",
            timestamp: eff?.timestamp || "",
            status: eff?.status || "",
            ...
            amountRaw,
            kind,
            actionLabel: txEventActionLabel,
          });
        }
      }
      ...
      if (txHasMatch && tx?.digest) {
        ...
        if (txEventActionLabel) signals.push(txEventActionLabel.toLowerCase());
    ...
    while (hasPreviousPage && scannedTx < MAX_SCAN_TX) {
      ...
      if (!matchedTx.size && scannedPages >= EARLY_EMPTY_PAGE_THRESHOLD) {
        stoppedForTargetedFallback = true;
        break;
      }
    ...
    if (!transferRows.length && !eventRows.length && !objectRows.length) {
      try {
        ...
        if (candidates.length) {
          ...
          const topDigests = metaRows.map((row) => row.digest).filter(Boolean).slice(0, 60);
    ...
    if (stoppedForTargetedFallback) {
      notes.push(`No matches in initial ${fmtNumber(scannedTx)}-tx fast scan; switched to object-linked fallback.`);
    }
